...
Variant *LinkerScript::getOrCreateVariant(StringRef name, bool &created) {
    Variant *&ref = nameToVariant[name];
    if (!ref) {
        ref = make<Variant>(name);
        created = true;
    } else {
        created = false;
    }
...
void LinkerScript::organizeVariants(void) {
    for (auto &it : nameToVariant) {
        it.second->organizeSections(symtab);
    }
}
  ...
  for (const SectionPattern &pat : cmd->sectionPatterns) {
    ...
    for (size_t i = 0, e = sections.size(); i != e; ++i) {
      ...
      if (isa<InputSection>(sec) &&
       ...
       // if (sec->file && cmd->variant) {
       //     std::cerr << sec->file->getName().str()
       //               << ":" << getFilename(sec->file).str()
       //               << "/" << sec->name.str() << " => "
       //               << cmd->matchesFile(sec->file)
...
void LinkerScript::processSectionCommands() {
  ...
  for (BaseCommand *base : sectionCommands) {
      if (auto *sec = dyn_cast<OutputSection>(base)) {
          ...
          if (sec->variant != nullptr) {
              if (seen.contains(sec->variant))
                  continue;
              seen.insert(sec->variant);
              for (auto *s : sec->variant->sections)
...
void LinkerScript::output(InputSection *s) {
  ...
  size_t multivariantFunctionSize = MVL::getMultivariantSectionMaxSize(cast<InputSectionBase>(s));
  uint64_t before = advance(0, 1);
  uint64_t sectionSize = multivariantFunctionSize > 0 ? multivariantFunctionSize : s->getSize();
  uint64_t pos = advance(sectionSize, s->alignment);
  s->outSecOff = pos - sectionSize - ctx->outSec->addr;
  pos = advance(s->addend, 1);
...
void LinkerScript::assignOffsets(OutputSection *sec) {
  ...
  // If we are entering a multivariant output section we want to make sure
  // that these sections overlay.
  if(MVL::isMultivariantOutputSection(sec)){
    uint64_t overlayVA = MVL::getOverlayMultivariantSectionVA(sec);
    if (overlayVA > 0)
      dot = overlayVA;
  }
  switchTo(sec);
  ...
  if (sec->addrExpr) {
      // std::cerr << "ADDR " << sec->name.str() <<(void*) sec->addrExpr().getValue() << " " << (void*) savedDot << "\n";
      // ctx->lmaOffset = alignTo(dot, sec->alignment) - savedDot;
      // std::cerr << "  " << (int64_t) ctx->lmaOffset << "\n";
      sec->lmaAddr = savedDot;
      sec->lmaSet = true;
  } else if (sec->lmaExpr){
      // FIXME: This is very bad (chris) : ctx->lmaOffset = sec->lmaExpr().getValue() - dot;
      sec->lmaAddr = sec->lmaExpr().getValue();
      sec->lmaSet = true;
...
void LinkerScript::adjustSectionsAfterSorting() {
  ...
  for (BaseCommand *base : sectionCommands) {
    if (auto *sec = dyn_cast<OutputSection>(base)) {
      ...
      // Output Section can mask their allocation flags (for Variants)
      sec->flags &= sec->flags_mask;
    }
  }
