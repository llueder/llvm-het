...
private:
  ...
  OutputSection *readVariantSectionDescription();
  OutputSection *readOutputSectionDescription(StringRef outSec);
  std::vector<BaseCommand *> readOverlay();
  std::vector<BaseCommand *> readVariant();
...
std::vector<BaseCommand *> ScriptParser::readVariant() {
    bool variantCreated;
    std::vector<BaseCommand *> v;
    StringRef OverlayName = next();
    StringRef LastPoint;
    auto Location = getCurrentLocation();

    // Get or Create a new Variant
    Variant *variant = script->getOrCreateVariant(OverlayName, variantCreated);
    ...
    if (consume("(")) {
        while (!errorCount() && !consume(")")) {
            ...
            if (StringRef(option).startswith("equal=")) {
                // std::cout << "EQUAL SECTION " << option.substr(6) << std::endl;
                variant->setEqualOutputSection(option.substr(6));
                continue;
            }

            auto flags = StringSwitch<uint64_t>(option)
                .Case("clash=jump_table", Variant::JUMP_TABLE_ON_CLASH)
                .Case("noexec",           Variant::REMOVE_EXECUTE_FLAG)
                .Default(-1);
            if (flags == -1UL) {
                error("Unkown VARIANT option: `" + option + "'");
                continue;
            }
    ...
    variant->startVariant(/* primary = */ variantCreated);
    expect("{");
    while (!errorCount() && !consume("}")) {
        OutputSection *os = readVariantSectionDescription();
        variant->addSection(os);
    }
    variant->stopVariant();

    return variant->grabCommands();
}
...
void ScriptParser::readSections() {
  ...
  while (!errorCount() && !consume("}")) {
    ...
    if (tok == "OVERLAY") {
      ...
      // The variant keyword reconfigures the linker to be prepared
      // for duplicate defitions.
      config->allowMultipleDefinition = true;
      
      for (BaseCommand *cmd : readVariant())
          v.push_back(cmd);
      continue;
    } else if (tok == "INCLUDE") {
      readInclude();
...
OutputSection *ScriptParser::readVariantSectionDescription() {
    OutputSection *cmd =
        script->createOutputSection(next(), getCurrentLocation());
    cmd->inOverlay = true;
    expect("{");
    while (!errorCount() && !consume("}")) {
        ...
        if (consume("INPUT_SECTION_FLAGS"))
            std::tie(withFlags, withoutFlags) = readInputSectionFlags();
        // SHF_STRINGS cannot be matched by Variant Patterns
        withoutFlags |= SHF_STRINGS;
        cmd->sectionCommands.push_back(
            readInputSectionRules(next(), withFlags, withoutFlags));
    }
    return cmd;
