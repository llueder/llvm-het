...
static uint64_t getSymVA(const Symbol &sym, int64_t &addend) {
  ...
  case Symbol::DefinedKind: {
    ...
    SectionBase *sb = MVL::getMultivariantLocalSymbolSectionBase(&sym);
    uint64_t va = sb != nullptr ? sb->getVA(offset) : isec->getVA(offset);
...
void Symbol::resolve(const Symbol &other) {
    // std::cerr
    //   << (other.file ? other.file->getName().str() : "") << " | "
    //   << this->kind() << toString(*this) << "->" << other.kind() << toString(other)
    //   << " W" << other.isWeak() 
    //   << " C" << other.isCommon()
    //   << " U" << other.isUndefined()
    //   << " L" << other.isLazy()
...
int Symbol::compare(const Symbol *other) const {
  ...
  if (other->isWeak()) {
      // If both symbols are weak, we return -2
      if (isWeak())
          return -2;
      return -1;
  }
...
void Symbol::resolveDefined(const Defined &other) {
  // -1: old wins, 1: new wins, 0: a conflict
  // -2: Old and new are weak
  int cmp = compare(&other);
  if (cmp > 0)
    replace(other);
  else if (config->allowMultipleDefinition &&
           ((cmp == 0)       // Duplicate Defined Symbol
            || (cmp == -2))) { // Duplicate Weak Symbol
      // Append this Symbol to the other one
      Defined *p;
      for (p = (Defined*) this;
           p->next != nullptr;
           p = p->next) {}
      p->next = new Defined(other);
  }
  else if (cmp == 0){
    // If we configured multivariant linking we allow duplicate symbols in first place.
    if(config->multivariantLinking && this->file && other.file){
      bool symbolOutOfMultivariantFile = std::find_if(
        config->multivariantObjects.begin(),
        config->multivariantObjects.end(),
        [&](const std::pair<int, llvm::StringRef>& file) -> bool{
          return file.second.str() == this->file->getName().str() &&
                  file.second.str() == other.file->getName().str();
        }
      ) != config->multivariantObjects.end();

      if(symbolOutOfMultivariantFile){
        replace(other);
        return;
      }
    }
