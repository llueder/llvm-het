...
static bool compareSections(const BaseCommand *aCmd, const BaseCommand *bCmd) {
  ...
  // Meta-section has to be after every generation section so we process it at the very last.
  if(a->name.str().find(MVL::META_SECTION_NAME) != std::string::npos){
    if(b->name.str().find(MVL::MVS_PREFIX) != std::string::npos)
      return false;
  }
  else if(b->name.str().find(MVL::META_SECTION_NAME) != std::string::npos){
    if(a->name.str().find(MVL::MVS_PREFIX) != std::string::npos)
      return false;
  }
...
std::vector<PhdrEntry *> Writer<ELFT>::createPhdrs(Partition &part) {
  ...
  for (OutputSection *sec : outputSections) {
    if (!needsPtLoad(sec))
      continue;
    // Skip multivariant sections, they have been processed already.
    ...
    if (!(load && newFlags == flags && sec != relroEnd &&
      ...
      // std::cerr << "new region " << sec->name.str()
      //           << !!load << (newFlags == flags) << (sec != relroEnd)
      //           << (sec->memRegion == load->firstSec->memRegion)
      //           << sameLMARegion
      //           << (sameLMARegion || load->lastSec == Out::programHeaders)
      //           << "\n";
  ...
  MVL::addMultivariantSegmentHeaders(ret);
  return ret;
}
...
static uint64_t computeFileOffset(OutputSection *os, uint64_t off) {
  ...
  if (os->lmaSet) {
      // std::cout << os->name.str() << " " << std::hex << first->name.str() << first->offset << " " << (os->lmaAddr - first->getLMA()) << std::endl;
      return first->offset + os->getLMA() - first->getLMA();
  }
...
static void checkOverlap(StringRef name, std::vector<SectionOffset> &sections,
  ...
  for (size_t i = 1, end = sections.size(); i < end; ++i) {
    ...
    if (type == AddressType::VMA && a.sec->inOverlay && b.sec->inOverlay)
      continue;

    // Allow the overlap of load memory addresses for multivariant sections too.
    // These types of sections and their corresponding segments will never ever
    // be loaded at the same - they get replaced by each other.
    // So in fact, there is no problem in it.
    if(
      type == AddressType::LMA &&
      a.sec->inOverlay &&
      b.sec->inOverlay &&
      MVL::isMultivariantOutputSection(a.sec) &&
      MVL::isMultivariantOutputSection(b.sec)
    )
      continue;
...
template <class ELFT> void Writer<ELFT>::checkSections() {
  ...
  checkOverlap("file", fileOffs, AddressType::FILE);
  ...
  checkOverlap("virtual address", vmas, AddressType::VMA);
  ...
  checkOverlap("load address", lmas, AddressType::LMA);
}
