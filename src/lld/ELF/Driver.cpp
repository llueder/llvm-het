...
static void parseMultivariantObjects(opt::InputArgList &args, std::vector<std::pair<int, StringRef>> &objects){
  auto test = args::getStrings(args, OPT_multivariant_objects);
  for(auto arg: test){
    auto splittedArg = arg.split(":");
    objects.emplace_back(std::make_pair(std::stoi(splittedArg.first.str()), splittedArg.second));
  }
}
...
static void readConfigs(opt::InputArgList &args) {
  ...
  config->multivariantLinking = args.hasFlag(OPT_multivariant_linking, OPT_no_multivariant_linking, false);
  parseMultivariantObjects(args, config->multivariantObjects);
...
void LinkerDriver::createFiles(opt::InputArgList &args) {
  ...
  for (auto *arg : args) {
    ...
    case OPT_no_whole_archive:
      ...
      break;
    case OPT_ignore_comdat:
        config->ignoreComdat = true;
        break;
    case OPT_no_ignore_comdat:
        config->ignoreComdat = false;
        break;
    ...
    case OPT_end_lib:
      ...
      break;
    case OPT_push_state:
      stack.emplace_back(config->asNeeded, config->isStatic, inWholeArchive, config->ignoreComdat);
      break;
    case OPT_pop_state:
      if (stack.empty()) {
      ...
      std::tie(config->asNeeded, config->isStatic, inWholeArchive, config->ignoreComdat) = stack.back();
      stack.pop_back();
      break;
    }
  }
...
template <class ELFT> void LinkerDriver::link(opt::InputArgList &args) {
  ...
  for(auto file: files){
    bool isMultivariantFile = std::any_of(
      config->multivariantObjects.begin(),
      config->multivariantObjects.end(),
      [file](const std::pair<int, StringRef> &object){
        return file->getName() == object.second;
      }
    );
    if(isMultivariantFile){
        for(auto sym: file->symbols) {
            MVL::recordMultivariantLocalSymbol(sym);
        }
    }
  }
    ...
    for (InputFile *f : objectFiles)
      for (InputSectionBase *s : f->getSections())
        if (s && s != &InputSection::discarded){
          MVL::recordMultivariantSection(s);
...

  {
    ...
    // Create multivariant output sections.
    MVL::processMultivariantSections();
