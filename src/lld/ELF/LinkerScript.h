...
struct SymbolAssignment : BaseCommand {
    SymbolAssignment(StringRef name, Expr e, std::string loc, std::string exprString="?")
      : BaseCommand(AssignmentKind), name(name), expression(e), location(loc),
        commandString(name.str() + " = " + exprString){}
...
public:
  InputSectionDescription(StringRef filePattern, uint64_t withFlags = 0,
      ...
      : BaseCommand(InputSectionKind), filePat(filePattern),
        withFlags(withFlags), withoutFlags(withoutFlags), variant(nullptr) {}
...
class LinkerScript final {
  ...
  llvm::DenseMap<StringRef, Variant *> nameToVariant;
...
public:
  ...
  Variant *getOrCreateVariant(StringRef name, bool &created);
  void organizeVariants(void);
